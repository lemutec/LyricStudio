...
internal abstract class VideoRendererBase : IMediaRenderer, ILoggingSource
{
    ...
    protected void ClearCaptions() =>
        Dispatcher.UIThread.InvokeAsync(() => MediaElement?.CaptionsView?.Reset(), DispatcherPriority.Background);
    ...
    // ava not sup
    private void ApplyRenderTransforms(VideoBlock b)
    {
        var videoView = MediaElement?.VideoView;
        ...
        if (videoView.RenderTransform is TransformGroup renderTransforms)
        {
            scaleTransform = renderTransforms.Children[0] as ScaleTransform;
            rotateTransform = renderTransforms.Children[1] as RotateTransform;
        }
        else
        {
            renderTransforms = new TransformGroup();
            scaleTransform = new ScaleTransform(1, 1);
            rotateTransform = new RotateTransform(0, 0.5, 0.5);
            renderTransforms.Children.Add(scaleTransform);
            renderTransforms.Children.Add(rotateTransform);

            videoView.RenderTransform = renderTransforms;
        }
