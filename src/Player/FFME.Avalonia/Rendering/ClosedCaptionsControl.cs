...
internal sealed class ClosedCaptionsControl : Viewbox
{
    ...
    private void InitializeComponent()
    {
        ...
        CaptionsGrid = new Grid { UseLayoutRounding = true, Focusable = false };
        Child = CaptionsGrid;
    ...
    private void PaintBuffer()
    {
        ...
        for (var r = 0; r < ClosedCaptionsBuffer.RowCount; r++)
        {
            for (var c = 0; c < ClosedCaptionsBuffer.ColumnCount; c++)
            {
                ...
                border.IsVisible = string.IsNullOrEmpty(cell.Text);

                if (!border.IsVisible)
                    continue;

                block.Text = cell.Text;
                block.FontStyle = cell.IsItalics ? FontStyle.Italic : FontStyle.Normal;
